        ...
        if (input_code.equals(code)) {
            switch (type_auth) {
                ...
                case Constants.REGISTER: {
                    ...
                    getServletContext().getRequestDispatcher("/WEB-INF/views/congrat.jsp").forward(req, resp);
                    break;
                }
                case Constants.FORGOT_PASSWORD: {
                    userService.updateUserPassword(email, password);

                    getServletContext().getRequestDispatcher("/WEB-INF/views/password_updated.jsp").forward(req, resp);
                    break;
                }
            }
        } else {
